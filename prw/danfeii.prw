...
//compatibilizar
#define MASCPESOL "@E 999999.99"
#define MASCPESOB "@E 999999.99"
/*/
...
// Variaveis AUTO-ROBERLO
Local nOpBol    := 0
Private oImpBol := Nil
Private nNumBol := 0
Private aRecBol := {}
Private lBolDanfe := GetNewPar("MV_BOLDANF", .F.)
...
private oRetNF
Private lPortalRep	:= isInCallStack("u_danfepdf")//Alterado Lucilene SMSTI - Envio danfe por email. 15.05.21
...
If nTipo <> 1
	...
	If !lPortalRep //Alterado Lucilene SMSTI - Envio danfe por email. 19.07.19
		oDanfe:lServer := oSetup:GetProperty(PD_DESTINATION)==AMB_SERVER
	Endif
	// ----------------------------------------------
	// Define saida de impressão
	// ----------------------------------------------
	If !lPortalRep //Alterado Lucilene SMSTI - Envio danfe por email. 
		...
		ElseIf oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
			...
			If Upper(AllTrim(oDanfe:cPathPDF)) == "C:\"
				If U_ChecaDir("c:\danfe\")
					oDanfe:cPathPDF := "c:\danfe\"
					oSetup:aOptions[PD_VALUETYPE] := "c:\danfe\"
				Else
					oDanfe:cPathPDF := GetTempPath()
					oSetup:aOptions[PD_VALUETYPE] := GetTempPath()
				EndIf
			EndIf
	...
	If lPergunte
		If lPortalRep //Adicionado Lucilene SMSTI - DANFE por email -  15.05.21
			lEnd:= .F.
			conout("cIdEnt: entrou aqui")
...
// Boletos AUTO-ROBERLO
If !lBolDanfe .and. Len(aRecBol) > 0 .and. lExistNfe .and. !lPortalRep //Alterado Lucilene SMSTI - 15.05.21
	aEval(aRecBol, {|aBol| U_ImpBolF2(aBol[1], aBol[2], @oImpBol, @oSetup, @nNumBol)})
	...
	While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
		...
		If (cAliasSF3)->(Eof())
			...
			For nX := 1 To nLenNotas
				...
				aXML[nX][4] := EncodeUTF8(aXML[nX][4])
				If !Empty(aXML[nX][2])
					If !Empty(aXml[nX])
						cAutoriza   := aXML[nX][1]
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						Else
							...
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								...
								EndiF
								// Boletos AUTO-ROBERLO
								aAdd(aRecBol, {SF2->F2_DOC, SF2->F2_SERIE})
...
If Type("oTransp:_Vol")<>"U"
	...
	Else
		...
		If  Type("oTransp:_Vol:_PesoB:TEXT") # "U"
			nPesoB := Val(oTransp:_Vol:_PesoB:TEXT)
		EndIf
		If Type("oTransp:_Vol:_PesoL:TEXT") # "U"
			nPesoL := Val(oTransp:_Vol:_PesoL:TEXT)
		EndIf
	EndIf
	If nPesoB > 0
		aTransp[15] := AllTrim(Transform(nPesoB, MASCPESOB))
	EndIf
	If nPesoL > 0
		aTransp[16] := AllTrim(Transform(nPesoL, MASCPESOL))
	EndIf
...
If lPortalRep //Alterado Lucilene SMSTI - Envio danfe por email. 15.05.21
	//Passado tamanho fixo, pois a função GetTextWidth() não pode ser executada por job.
	aTamCol:= {50,185,40,16,24,14,35,53,38,36,30,22,31,29}
Else
...
// AUTO-ROBERLO - compatibilizar
// para imprimir boletos no mesmo PDF
If lBolDanfe .and. !lPortalRep //Alterado Lucilene SMSTI - 15/05/21
	U_ImpBolF2(StrZero(Val(oIdent:_NNf:Text), 9), PadR(oIdent:_Serie:Text, 3), oDanfe)
EndIf

// AUTO-ROBERLO - compatibilizar
// para atualizar numero da NF na CiaShop
If oIdent:_TpNf:Text == "1" .and. GetNewPar("MV_INTCS", .F.) .and. !lPortalRep //Alterado Lucilene SMSTI - 15.05.21
	U_NFCiaShop(StrZero(Val(oIdent:_NNf:Text), 9), PadR(oIdent:_Serie:Text, 3), SubStr(oNF:_InfNfe:_ID:Text, 4), If(Type("oIdent:_dhSaiEnt") # "U", oIdent:_dhSaiEnt:Text, Nil))
EndIf
Return(.T.)
	...
	If oWS:RETORNANOTASNX()
		If Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5) > 0
			For nX := 1 To Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5)
				...
				//Alteração Emerson SMSTI chamado 24466 para fechar a tag em notas antigas com problema:
				If right(alltrim(cDHRecbto),1) != '>'
					cDHRecbto += '>'
				EndIF
				//Fim Alteração Emerson SMSTI
