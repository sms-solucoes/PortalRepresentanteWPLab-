...
//Seleciona as condições de pagamento disponíveis no combo
	...
	cCondPag+='required="" aria-required="true" onchange="javascript:vldDescto()" >' 
		...
		For i:=1 to Len(aItens)
			If ValType(aItens[i,1]) == 'C'
				...
				If !Empty(nAcresc) .and. i = 1
					aadd(aCabSCJ,{"CJ_XACRES",nAcresc ,Nil})
				Endif
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦Funçäo    ¦ VldAcresc	¦ Autor ¦ Lucilene Mendes   ¦ Data ¦ 23.08.19 ¦¦¦
¦¦+----------+------------------------------------------------------------¦¦¦
...
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
*/
// Em 17/03/21 removida o teste de modalidade para geracao de acrescimo - chamado 24902 - Pedro / SMS
User Function VldAcresc()
	Local cHtml
	Local cModCC	:= ''
	Local cCondicao	:= Alltrim(HttpPost->condicao)
	Local cModali	:= Alltrim(HttpPost->modalidade)
	Local nTotal	:= Val(StrTran(StrTran(HttpPost->valor,'.',''),',','.'))
	Local cGrupos	:= Alltrim(HttpPost->grupos)
	Local cCliente	:= Alltrim(HttpPost->cliente)
	Local nValMin	:= 0
	Local nAcrMin 	:= 0
	Local nPzMedio 	:= 0
	...
	cModCC:= GetNewPar("MV_MODCART", "005,006")
	nValMin:= Posicione("SE4",1,xFilial("SE4")+cCondicao,"E4_VALMIN")
	cCliEspec:= Posicione("SA1",1,xFilial("SA1")+cCliente,"A1_YESPECI")

	if (cCliEspec <> "S")
		nAcrMin:= SE4->E4_XACRES
		nPzMedio:= SE4->E4_XPRAZME

		//Grupo especial
		If SE4->E4_XGRPESP $ cGrupos
			nValMin := SE4->E4_XVALMIN
		Endif

		// Em 17/03/21 removida o teste de modalidade para geracao de acrescimo - chamado 24902 - Pedro / SMS
		//	If nValMin > 0 .and. !(cModali $ cModCC) .and. nTotal < nValMin
		If nValMin > 0 .and. nTotal < nValMin
			If nAcrMin > 0 .and. nPzMedio > 0
				BeginSql Alias "_QZE4"
					SELECT * FROM
					%table:ZE4%
					WHERE
					ZE4_FILIAL 	= %exp:xFilial("ZE4")% AND
					ZE4_VLRMIN	<= %exp:nTotal% AND
					ZE4_VLRMAX  >= %exp:nTotal% AND
					D_E_L_E_T_ 	= ' '
				EndSql
				aQry := GetLastQuery()
				// conout('addOrc - Query:'+cValToChar(aQry[2]))
				nPzMdZE4	:= _QZE4->ZE4_PZMED
				_QZE4->(dbCloseArea())
				// conout('addOrc - nPzMdZE4:'+cValToChar(nPzMdZE4))

				nDifPzMd	:= (nPzMedio - nPzMdZE4)* nAcrMin
				// conout('addOrc - nDifPzMd:'+cValToChar(nDifPzMd))
			Endif
		Endif
	Endif

	cHtml:=  AllTrim(Transform(Abs(nDifPzMd), "@E 99.99999"))+'|'+ cValtoChar(nDifPzMd)
