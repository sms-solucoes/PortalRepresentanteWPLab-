...
If lEstoque
	...
	*/			
	...
	aItens := {	{"Item","CK_ITEM","*","text-left","C",.F.,.F.,.F.,"",.F.,""},;
				...
				{"Saldo","SALDO_EST","*"," text-right","N",lEdit,.T.,.F.,"0",.F.,""}}				
...
Else
	...
	Posicione("SE4",1,xFilial("SE4")+SCJ->CJ_CONDPAG,"E4_DESCRI")	
	...
	If lEdit
		...
		End
	Else
	 	cCondPag+='	<option value="'+SCJ->CJ_CONDPAG+'">'+SCJ->CJ_CONDPAG+" - "+SE4->E4_DESCRI+'</option>'		
	Endif	
	cCondPag+='</select>'	                             
...
Endif
	...
	While SCK->(!Eof()) .and. SCK->CK_FILIAL == SCJ->CJ_FILIAL .and. SCK->CK_NUM == SCJ->CJ_NUM 
			...
			Else	
				...
				Do Case
					...
					Case aItens[nLin][5] == 'N'
						...
						ElseIf aItens[nLin,2] == "iCK_PRCVEN"	
							xValue := Alltrim(TransForm(SCK->CK_PRCVEN,PesqPict("SCK","CK_XICMST")))
						Elseif aItens[nLin,2] == "CK_PRCVEN"	
							xValue := Alltrim(TransForm(SCK->CK_PRUNIT,PesqPict("SCK","CK_PRCVEN")))
						Elseif aItens[nLin,2] == "CK_VALOR"	
							xValue := Alltrim(TransForm(SCK->(CK_VALOR+CK_XVALIPI+CK_XICMST),PesqPict("SCK",aItens[nLin,2])))
						Elseif aItens[nLin,2] == "SALDO_EST"	
							xValue := Alltrim(cvaltochar(u_VerEstoque(SCK->CK_PRODUTO)))
						Else
		...
		If lEdit .and. !lEstoque
			...
			cItensHid += '<input type="hidden" class="" id="ULTRAPASSA'+cItem+'" name="ULTRAPASSA'+cItem+'" value="2">' //'+Iif(!Empty(SB1->B1_XULTMAX),SB1->B1_XULTMAX,'1')+'
	...
	nTTotal+= nTFrete
	nTAcresc+= Round(nTtotal * (SCJ->CJ_XACRESC/100),2)
	nTTotal+= nTAcresc
	...
	If !lEstoque
			...
			cOrcItens += '<input type="hidden" name="ACRESCIMO" id="ACRESCIMO" value='+cvaltochar(SCJ->CJ_XACRESC)+' />'
			cOrcItens += '<input type="hidden" name="VALMIN_COND" id="VALMIN_COND" value='+cvaltochar(SE4->E4_XVALMIN)+' />'
			cOrcItens += '<input type="hidden" name="JUROS_COND" id="JUROS_COND" value='+cvaltochar(SE4->E4_XPERJUR)+' />'
